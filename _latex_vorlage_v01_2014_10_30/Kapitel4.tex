...
Im Allgemeinen kann das Modell eine statische Umgebung relativ gut beschreiben. Dieser Teil wurde übrigens erklärt, während die Implementierung in Kapitel~\ref{Kapitel:Implementierung} erläutert wurde. Hier werden hauptsächlich einige kleine Details erläutert, die auf der Karte angezeigt werden. Hierbei wird hauptsächlich auf einige kleine Details beschränkt, die auf der gitterbasierten Karte angezeigt werden.
\subsubsection{Freiraum hinter Hindernisse}
\label{Freiraum hinter Hindernisse}
Theoretisch sollte sich hinter dem Hindernis ein unbekannter Bereich befinden. Das Ergebnis der Visualisierung ist jedoch, wie in Abbildung~\ref{fig:FreiraumHinterHindernisse} gezeigt, dass der Bereich links vom Hindernis im Bild als Freiraum bezeichnet wird. 
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.5\textwidth]{pics/FreiraumHinterHindernisse.pdf}
	\caption{Freiraum hinter Hindernisse}
	\label{fig:FreiraumHinterHindernisse}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.5\textwidth]{pics/loss_data.pdf}
...
\subsection{Laufgeschwindigkeit des implementierten Modells}
Da ROS über Tools verfügt, mit denen die Frequenz von Topic überprüft werden kann, wird die Berechnungszeit oder die Berechnungsgeschwindigkeit des Modells hier indirekt durch die Frequenz beschrieben. Wie in Abbildungen~\ref{fig:F_PCL}~\ref{fig:F_UTM}~\ref{fig:F_can_gsps} gezeigt, gehört die niedrigste Frequenz zu den Informationen aus der Punktwolke, die etwa 25 Hz beträgt. Die begrenzt den Maximalwert der Laufgeschwindigkeit des Modells.
\begin{figure}[htbp] 
	\centering
	\includegraphics[width=0.9\textwidth]{pics/F_PCL.pdf}
	\caption{Frequenz von Information über Punktwolke}
	\label{fig:F_PCL}
\end{figure}
\begin{figure}[htbp] 
	\centering
	\includegraphics[width=0.9\textwidth]{pics/F_UTM.pdf}
	\caption{Frequenz von Information über UTM-Koordinaten}
	\label{fig:F_UTM}
\end{figure}
\begin{figure}[htbp] 
	\centering
	\includegraphics[width=0.9\textwidth]{pics/F_can_gsps.pdf}
	\caption{Frequenz von Information über Nickelwinkel}
	\label{fig:F_can_gsps}
\end{figure}
Um die Laufgeschwindigkeit zu messen, wird die Frequenz der Modellausgangsdaten gemessen. Wie in Abbildung A gezeigt, beträgt die Ausgangsfrequenz etwa 12,5 Hz. 
\begin{figure}[htbp] 
	\centering
